...
\clearpage{}
\begin{abstract}
The aim of this project was to build a system to simulate the interaction of CMOS transistors, with the following
...
a true/false output, and how lots of these boxes in interesting combinations
...
blocks of a computer's processor. The layer underneath this is built up of Complementary Metal-Oxide
...
a plain string through a series of intermediary formats, until a visual
representation is produced.

Initially, this document will discuss the requirements of and use cases of a program to reason about CMOS, and we will then 
define key concepts, such as the fundamental propositional logic syntax and the semantic meanings usually associated
with logic gates, and the definition of the two different transistors
that form CMOS. After this, the algorithms to be used will be defined. On closer inspection, their composition has
...
will then be parsed and internally converted to a canonical form (DNF) which
...
\end{align}
where $\Sigma$ is some accepted alphabet of variables (in 
this program, as described by the Perl-Compatible Regex \verb&^(?!out|and|or$)([A-Za-z0-9]+)&,
...
the program, as well as \verb+and+ and \verb+or+ which are keywords in
...
and are represented in the program by the strings \code{1} and \code{0}
...
rules: 
\begin{itemize}
\item $E=\neg F$: $E$ is evaluated as true iff $F$ is false (corresponding to equation \ref{eq:neg}) 
\item $E=F\lor G$: $E$ is evaluated as true iff $F$ is true or $G$ is
true (corresponding to equation \ref{eq:disj}) 
\item $E=F \land G$: $E$ is evaluated as true iff $F$ is true and $G$ is true (corresponding
...
the output wire, referred to in this document as \code{Result}.
...
if the given fragment evaluates to true, as defined in \vref{eq:neg}
...
The operation is, in the worst case, exponential in the number of variables used, $\mathcal O \left( 2^n\right)$.  The expression $\oplus_{i=1}^n v_i$ will have $2^{n-1}$ clauses, each containing $n$ variables which must each be individually processed by the transistor-generation algorithm.
...
being connected to the \code{Source} twice, and $c$ and $\neg c$
being connected to the drain twice, however, by merging both sets of
gates, connections are then formed such that \code{Result} is always
driven, either with a low or high potential.
...
In the worst case, for the drain having $2^n$ connections of $n$ variables, as seen in the case of repeated application of the xor function, at the $i^\text{th}$ stage, $2^{i-1}$ transistors may be combined, effectively forming a binary tree. Let $T:\mathbb N \rightarrow \mathbb R$ be the number of steps required to process $n$ nodes in the worst case,  this give $T(2^n) = T(k) = \mathcal O (k) + 2T\left(\sfrac k2\right)$, which by the Master Theorem, first discussed in \cite{bentley1980general}, therefore runs in $\Theta\left(k\log k\right) = \mathcal O \left(n2^n\right)$.  However, by application of the Birthday Problem, assuming that $\mathcal O \left(2^{\sqrt{n}}\right) $ entries are needed for a collision in a space of $2^n$ elements, this makes finding duplicate gates much less likely, and substantially reduces the number of wires that must be explored.
...
Initially, potential was going to be modelled using the \code{Option{[}Boolean{]}} type,
...
This runs in constant time, within some factor of $\sfrac32$, as each side of the network must be processed.
...
Expressions are then parsed using Scala's Packrat parsing library \cite{packrat}.
...
assignment function $\mathcal{A}$, as discussed in \vref{eq:assi}.
After parsing, but before the gate implementation (see \vref{sub:Gates-and-Wires}),
a modified version of the Quine-McCluskey algorithm (see \vref{sub:Normal-Forms})
...
capacity, regardless of the specific grammar used.  This library has the
...
As discussed in Quine and McCluskey's papers, this algorithm runs in $\mathcal O\left(3^n\right)$, in the worst case, $\left(\mathcal{O}\left(\sfrac{3^{n}}{n}\right)\right)$ prime implicants will be generated alone.  The problem that it solves is effectively in NP, though sub-exponential approximation algorithms are available.
...
\clearpage\section{Comparison to a Compiler}
...
expression is preferred.  The visual output \footnote{Reproduced in appendix} shown is used to demonstrate the difference
...
\begin{table}
\begin{tabularx}{\linewidth}{p{2cm}XXr}
\toprule
Input                                       & Driven Output                                  & Undriven Output                                 & Transistors \\
\midrule
$\neg a$                                    &   \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{notdriven}}  & \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{notundriven}} & 2 \tabularnewline
$(a\land b)\lor(\neg a\land\neg b\land c)$  &   \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{networklow}} & \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{networkhigh}} & 17 \tabularnewline
$a \land b \land c$                         &   \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{conjhigh}}   & \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{conjlow}}     & $6+2$ \tabularnewline
$a \lor b \lor c$                           &  \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{disjhigh}}   & \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{disjlow}}     & $6+2$ \tabularnewline
$(a \lor b \lor c) \land (a \lor b \lor c)$ &   \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{disjhigh}}   & \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{disjlow}}     & $6+2$ \tabularnewline
$a \oplus b \oplus c$ &   \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{xordri}}   & \noindent\parbox[c]{\hsize}{\includegraphics[width=\linewidth]{xorund}}     & $6+2$ \tabularnewline
\bottomrule
\end{tabularx}
\caption{Program Output\label{tab:vis}}
\end{table}

\clearpage\section{Conclusions and Further Work\label{sec:Conclusions-and-Further}}
...
order of completion of the modules was intended to be parser, then transistor-generator,
followed by gate-drawer, and then optimisations (including Quine-McCluskey-related functions),
...
state being invalid, whereas the standard tri-state buffer~\cite{digsys}, seen in \vref{fig:tri},
where $\mathit{enb}\rightarrow(\mathit{in}\leftrightarrow\mathit{out})$,
and $\neg\mathit{enb}$ results in the output wire being undriven, is a commonly used gate.
...
\caption{Tri-state buffer\label{fig:tri}}
\end{figure}


Additionally, the structure of the transistors produced by the transistor
placing algorithm in \vref{alg:Converting-DNF-expression} is more
...
Finally, a future project may focus on converting the model and various
helpers to use multi-threaded implementations to speed up the various algorithms,
especially the transistor-generation algorithm, as each sequential group of transistors
could be independently generated and can therefore be added in some thread-safe manner to
the shared \code{Result}, \code{Source}, and/or \code{Drain} objects.  Operations traversing are also prime candidates for parallelisation, using a controller-slave pattern.

\subsubsection{View}

The data produced by the view is currently only exported in the PNG format, which is easily convertible to other bitmap formats, whereas a future version of this software
should be able to produce output in a wider variety of formats, and ideally,
...
of running the Quine-McCluskey algorithm (in the worst case, $\left(\mathcal{O}\left(\sfrac{3^{n}}{n}\right)\right)$ prime implicants will be generated alone). Additionally, unless the
...
\end{landscape}
\clearpage
\clearpage\section{Test Results}
The following are the images seen in \vref{tab:vis}, with details of the assignment required.
\begin{figure}[h]
        \centering
        \begin{subfigure}{0.3\textwidth}
			\includegraphics[width=\linewidth]{notdriven}
\caption{$\mathcal A (a) = \bot$}
        \end{subfigure}
~
        \begin{subfigure}{0.3\textwidth}
\includegraphics[width=\linewidth]{notundriven}
\caption{$\mathcal A (a) = \top$}
        \end{subfigure}
\caption{$\neg a$}
\end{figure}
\begin{figure}[h!] \centering \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{networklow}
\caption{$\mathcal A (a) = \bot, \mathcal A (b) = \bot, \mathcal A (c) = \top$} \end{subfigure} ~ \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{networkhigh}
\caption{$\mathcal A (a) = \top, \mathcal A (b) = \top, \mathcal A (c) = \top$} \end{subfigure}
\caption{$(a \land b)\lor(\neg a \land \neg b \land c)$}
\end{figure}
\begin{figure}[h!] \centering \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{conjhigh}
\caption{$\mathcal A (a) = \bot, \mathcal A (b) = \top, \mathcal A (c) = \top$} \end{subfigure} ~ \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{conjlow}
\caption{$\mathcal A (a) = \top, \mathcal A (b) = \top, \mathcal A (c) = \top$} \end{subfigure} 
\caption{$a \land b \land c$ with negation applied to the result}
\end{figure}
\begin{figure}[h!] \centering \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{disjhigh}
\caption{$\mathcal A (a) = \bot, \mathcal A (b) = \bot, \mathcal A (c) = \top$} \end{subfigure} ~ \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{disjlow}
\caption{$\mathcal A (a) = \top, \mathcal A (b) = \top, \mathcal A (c) = \top$} \end{subfigure} 
\caption{$a \lor b \lor c$ and $(a \lor b \lor c) \land (a \lor b \lor c)$ with negation applied to the result}
\end{figure}
\begin{figure}[h!] \centering \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{xordri}
\caption{$\mathcal A (a) = \top, \mathcal A (b) = \top, \mathcal A (c) = \top$} \end{subfigure} ~ \begin{subfigure}{0.47\textwidth}
\includegraphics[width=\linewidth]{xorund}
\caption{$\mathcal A (a) = \bot, \mathcal A (b) = \bot, \mathcal A (c) = \bot$} \end{subfigure} 
\caption{$a \oplus b\oplus c$}
\end{figure}
\end{document}
