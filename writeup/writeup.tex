...
\lstset{
    ...
    tabsize=2,%
    basicstyle=\renewcommand{\baselinestretch}{0.95}\tiny\ttfamily,%
...
conjunctions, disjunctions and negations, using a grammar described
as follows:\cite{intlog}
...
\end{align}
where $\Sigma$ is some accepted alphabet of variable (specifically,
...
\eIf{$\text{normal}<\text{doubleNegate}$} { 
...
Initially, potential was going to be simulated using an \code{Option{[}Boolean{]}},
with \code{Nothing} to simulate a transistor/wire that is not driven,
and \code{Some(true)} and \code{Some(false)} to indicate high and
low potential separately. This approach has also been reworked, to
...
\midrule
$\neg a$ & \includegraphics[width=0.1\linewidth]{notdriven} & \includegraphics[width=0.1\linewidth]{notundriven} & 2 \tabularnewline
$(a\land b)\lor(\neg a\land\neg b\land c)$ & \includegraphics[width=0.2\linewidth]{networklow} & \includegraphics[width=0.2\linewidth]{networkhigh} & 17 \tabularnewline
 &  & \tabularnewline
 &  & \tabularnewline
 &  & \tabularnewline
 &  & \tabularnewline
...
\lstlistoflistings
\subsection{Model}
\lstinputlisting[multicols=2,language=scala,caption=Variable.scala]{../src/model/Variable.scala}
\subsection{View}
\subsection{Controller}
\lstinputlisting[multicols=2,language=java,caption=Gui.java]{../src/controller/Gui.java}
\subsection{Helper}
\end{landscape}
\end{document}
